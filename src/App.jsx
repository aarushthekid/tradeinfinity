...
const BOT_API_URL = "trade-infinity-bot-production.up.railway.app";   // ← REPLACE THIS (your Render bot URL)
...
export default function TradeInfinity() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (plan === "pro") daysLeft = -1; // unlimited
  ...
  // ─── PAYMENT (Demo — shows how Cashfree will work) ───
  const handlePayment = async () => {
    if (!currentUser) return;
    // In Phase 5, this will open Cashfree checkout
    ...
    try {
      await fetch(`${SUPABASE_URL}/rest/v1/users?id=eq.${currentUser.id}`, {
        ...
        body: JSON.stringify({ plan: "pro", pro_start: new Date().toISOString() }),
      });
      setUserPlan("pro");
      alert("✅ Pro plan activated! You now have unlimited access to all strategies and Telegram alerts.");
    } catch (err) {
      alert("Payment simulation done! In production, this will use Cashfree.");
      setUserPlan("pro");
    }
  };
  ...
  const PlanBadge = () => {
    ...
    if (userPlan === "pro") { bg = "#00e89d"; text = "PRO"; color = "#06080d"; }
